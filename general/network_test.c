...
int main(int argc, char *argv[]) {
  ...
  for (int img = image_start; img < image_start + image_length; img++) {
    ...
    forward_prop_interval_equation_linear_conv(
        nnet, &input_interval, &output_interval, grad, wrong_nodes_map,
        &wrong_node_length, &full_wrong_node_length, equation_conv,
        equation_conv_err, &err_row_conv);
    ...
    if (is_overlap) {
      if (full_wrong_node_length == 0) {
        printf("Not implemented: At least one node needs to be able to be "
               "split to "
               "test the LP. \n");
        exit(1);
      }
      ...
      // split
      int depth = 0;
      is_overlap = split_interval_conv_lp(
          nnet, &input_interval, output_map, grad, wrong_nodes_map,
          &wrong_node_length, sigs, equation_conv, equation_conv_err,
