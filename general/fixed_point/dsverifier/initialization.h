...
void initialization(int intb, int fracb) {
  ...
  /* check if the dsverifier_scale exists */
  if ((dsverifier_scale == 0) || (dsverifier_scale == 1)) {
    dsverifier_scale = 1;
    return;
  }

  /** applying dsverifier_scale in dynamical range */
  if (dsverifier_min != 0) {
    dsverifier_min = dsverifier_min / dsverifier_scale;
  }

  if (dsverifier_max != 0) {
    dsverifier_max = dsverifier_max / dsverifier_scale;
  }
}
